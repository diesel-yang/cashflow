...
createApp({
  setup(){
    ...
    const dueJACK = computed(()=> monthRecords.value.filter(r=>r.entity==='restaurant' && r.settlement && r.settlement.status!=='paid' && r.settlement.due_to==='JACK').reduce((a,b)=>a+(r.settlement.remaining ?? r.amount),0));
    const dueWAL = computed(()=> monthRecords.value.filter(r=>r.entity==='restaurant' && r.settlement && r.settlement.status!=='paid' && r.settlement.due_to==='WAL').reduce((a,b)=>a+(r.settlement.remaining ?? r.amount),0));
    ...
    async function autoSettle(personKey, payAmount){
      ...
      for(const r of targets){
        const left = (r.settlement.remaining ?? r.amount);
    ...
    // P&L + Charts (single declaration of 'pl')
    ...
    function exportCSV(scope){
      ...
      for(const r of recs){
        const row = [
          ...
          r.settlement ? r.settlement.status : '', r.settlement ? r.settlement.due_to : '', r.settlement ? (r.settlement.remaining ?? '') : '',
